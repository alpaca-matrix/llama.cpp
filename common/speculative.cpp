    ...
    bool process(const llama_batch & batch_in) override {
        ...
        for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
            ...
            for (int32_t offset = 0; offset < n_rows; offset += n_ubatch) {
                ...
                // sanitize non-finite feature values before fusing. backends that stage f32
                // activations as f16 for the mat-mat kernels overflow on Laguna's massive
                // activation rows (attention-sink tokens, |x| ~ 1e6 in the pre-final-norm
                // residual). one poisoned row would otherwise NaN the whole drafter KV cache.
                {
                    size_t n_bad = 0;
                    ...
                    if (n_bad > 0) {
                        ...
                        if (!warned) {
                            SPC_WRN("sanitized %zu non-finite target feature values (f16 overflow on massive activations); "
