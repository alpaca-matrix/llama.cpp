...
llama_model_laguna::graph::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    // A DFlash drafter needs the pre-final-norm hidden state for every token, so
    // when unmasked nextn extraction is on the last layer must stay full-width
    // and the output-row gather moves below the capture. Without a drafter this
    // is false and the last layer keeps narrowing to the output rows as before.
